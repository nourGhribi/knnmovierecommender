...
package object utils {
  ...
  def normalizedDev_M3(X:CSCMatrix[Double]):CSCMatrix[Double]={
    ...
    val buildingNormDevs = for(((user,movie),value)<-X.activeIterator)
                              normDevBuilder.add(user, movie, normalize(value, averageRatingPerUser(user)))

    normDevBuilder.result()
  }

  def preprocessing(X:CSCMatrix[Double]):CSCMatrix[Double] = {
    val nbUsers = X.rows
    val nbMovies = X.cols
  ...
  def compute_KNN_similarities(X:CSCMatrix[Double], k:Int):CSCMatrix[Double] = {
    ...
    // ------ Preprocessing -----
    val checkedRs = preprocessing(X) // NbUsers * NbMovies
    // --------------------------
  ...
  def KNN_predictions(X:CSCMatrix[Double], Y:CSCMatrix[Double], k:Int): KNN_result = {
    ...
    val kSimilarities = compute_KNN_similarities(X,k) // NbUsers * NbUsers: (v, u)
